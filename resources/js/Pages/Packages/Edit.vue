...
const props = defineProps({ package: Object, routers: Array });

const form = useForm({
    name: props.package.name,
    mikrotik_id: props.package.mikrotik_id,
    rate_limit: props.package.rate_limit,
    price: props.package.price,
    local_address: props.package.local_address,
    remote_address: props.package.remote_address,
    description: props.package.description,
    is_public: props.package.is_public,
});
</script>

<template>
    <Head :title="'EDIT_PACKAGE_' + package.name" />
        ...
        <form @submit.prevent="form.put(route('dashboard.packages.update', package.id))" class="space-y-6 bg-surface/40 border border-primary/10 p-8 shadow-2xl">
            ...
            <div class="pt-6">
                <button type="submit" :disabled="form.processing" class="w-full bg-primary text-black font-black py-4 uppercase text-xs hover:bg-white transition flex items-center justify-center gap-2">
                    <Save :size="16" /> {{ form.processing ? 'SYNCING_TO_ROUTER...' : 'Save Changes' }}
                </button>
            </div>
        </form>
    </div>
