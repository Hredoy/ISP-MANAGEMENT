...
<template>
    ...
    <div class="max-w-3xl mx-auto font-mono text-primary">
        ...
        <form @submit.prevent="form.post(route('dashboard.packages.store'))" class="space-y-6 bg-surface/40 border border-primary/10 p-8 shadow-2xl">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="col-span-2 space-y-1">
                    <label class="text-[10px] uppercase font-bold text-primary/70">Package Name *</label>
                    <input v-model="form.name" type="text" placeholder="e.g. 10MBPS Unlimited" class="w-full bg-surface border border-primary/30 p-3 text-sm focus:border-primary outline-none transition">
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <input v-model="form.rate_limit" type="text" placeholder="10M/10M" class="w-full bg-surface border border-primary/30 p-3 text-sm outline-none">
                </div>
                ...
                <div class="space-y-1">
                    ...
                    <input v-model="form.local_address" type="text" placeholder="Ip Address" class="w-full bg-surface border border-primary/30 p-3 text-sm outline-none">
                </div>

                <div class="space-y-1">
                    <label class="text-[10px] uppercase font-bold text-primary/70">Remote Address</label>
                    <input v-model="form.remote_address" type="text" placeholder="Ip Address/Pool" class="w-full bg-surface border border-primary/30 p-3 text-sm outline-none">
                </div>
